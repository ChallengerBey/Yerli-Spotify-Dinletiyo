...
import { Play, Pause, SkipBack, SkipForward, ListMusic, Volume2, X, Heart, Shuffle, Repeat, Home, User, Library, Music, Maximize2, ChevronUp } from "lucide-react";
...
export function Player() {
  ...
  useEffect(() => {
    const handlePlaySong = async (event: Event) => {
      ...
      if (currentSong?.id === song.id) {
        ...
        // Automatically open full screen player
        ...
        try {
          ...
          if (currentUser) {
            ...
            // Now playing API'sine gönder (overlay için)
            updateNowPlaying(userData.id, song, 0, 0, true);
          }
        } catch (error) {
  ...
  const createAutoPlaylist = async (song: Song) => {
    try {
      ...
      const finalPlaylist = [firstSong, ...shuffledRest].slice(0, 50);
      ...
      <footer className="hidden lg:flex items-center justify-between glass-card border-t border-white/10 px-6 py-8 backdrop-blur-xl bg-black/90 h-30">
        <div className="flex items-center gap-4 w-1/4">
          {currentSong.imageUrl ? (
            ...
            <div className="w-14 h-14 rounded-xl bg-gray-800 flex items-center justify-center cursor-pointer" onClick={() => setIsFullScreen(true)}>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {currentSong.id !== '0' && (
                <span className={`px-1.5 py-0.5 text-xs rounded ${(currentSong.language || detectSongLanguage(currentSong.title, currentSong.artist)) === 'turkish'
                  ? 'bg-blue-500/20 text-blue-300'
                  : 'bg-green-500/20 text-green-300'
                  }`}>
                  {(currentSong.language || detectSongLanguage(currentSong.title, currentSong.artist)) === 'turkish' ? '🇹🇷' : '🇺🇸'}
                </span>
              )}
            </div>
