...
export const socialService = {
  getUsers: async (search: string = '', currentUserId?: string): Promise<SocialUser[]> => {
    ...
    try {
      ...
      console.log('Database error, returning empty list:', error);
    }
    return [];
  },
  ...
  getFriends: async (currentUserId?: string): Promise<SocialUser[]> => {
    ...
    try {
      ...
      console.log('Database error, returning empty friends:', error);
    }
  ...
  getRandomUser: async (currentUserId?: string): Promise<SocialUser> => {
    try {
      ...
      console.log('Database error, no random user:', error);
    }
    throw new Error('Şu anda keşfedilecek kullanıcı bulunamadı.');
  }
};