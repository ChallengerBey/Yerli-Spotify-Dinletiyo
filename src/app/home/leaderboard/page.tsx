...
export default function LeaderboardPage() {
  const { user } = useAuth();
  const [data, setData] = useState<{ leaderboard?: any[]; source?: 'live' } | null>(null);
  ...
  const fetchLeaderboard = async () => {
    ...
    try {
      ...
      // Sadece gerçekten sıralamada varsa "Senin sıralaman" göster
      if (user && responseData.leaderboard) {
        const userStats = responseData.leaderboard.find((entry: any) => entry.user_id === user.id);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      {user && !currentUserStats && data?.leaderboard && (
        <Card className="border-muted">
          ...
          <CardContent>
            <p className="text-muted-foreground">
              Henüz sıralamada değilsin. Müzik dinleyerek puan kazan ve listeye gir!
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            En çok müzik dinleyen kullanıcılar
          </CardDescription>
        </CardHeader>
        <CardContent>
