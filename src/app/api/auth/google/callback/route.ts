...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!clientId || !clientSecret) {
      console.error('Google OAuth env eksik. GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET tanımlı mı?');
      return NextResponse.redirect(new URL('/login?error=env_error', request.url));
    }
