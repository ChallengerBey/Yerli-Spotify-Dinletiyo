  ...
  useEffect(() => {
    const checkNowPlaying = async () => {
      try {
        ...
        if (!response.ok) {
          // API hata verirse overlay'i çökertme; sadece hata mesajını göster
        ...
        if (data?.nowPlaying) {
          // Şarkı duraklatılsa bile overlay bilgi göstersin (OBS'ta boş kalmasın)
